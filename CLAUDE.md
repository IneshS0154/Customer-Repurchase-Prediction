...
**Stack: plain R scripts** (switched from Python per lecturer requirement; `.R`, not
`.Rmd`, per the user's preference). An earlier Python version is kept for reference in
`legacy-python/` but is not part of the submission and should not be extended.
...
Run a script from a terminal with `Rscript notebooks/00_data_cleaning.R`, or open it in
RStudio and run line by line / the whole file (Cmd+Shift+Enter).

There is no test suite, linter, or build step — this is an analysis project. `notebooks/00_data_cleaning.R`
must be run first; every other script reads its output from `data/processed/`.

## Architecture

**Data flow:** `data/raw/` (raw UCI xlsx, gitignored) → `R/cleaning.R` (line-level quality
fixes) → `R/features.R` (aggregation to one row per customer) → `data/processed/*.csv`
(gitignored) → consumed independently by each `notebooks/0N_*.R`.

**The cutoff-date design is the central mechanic of the whole project** (`R/features.R`):
...
**`R/` holds the only shared logic** (`cleaning.R` loads/cleans raw invoice lines,
`features.R` builds the customer table via `build_customer_table()`). Everything else —
statistics, modelling, plots — lives inline in the scripts per task, since each one
corresponds to one graded task in the brief and should be independently readable/reviewable
by teammates. Don't move script analysis code into `R/` unless it's genuinely reused
across scripts. Section headers use `# ---- Title ----` (RStudio folds these).
...
avoid restructuring `R/` function signatures without checking what other notebooks call.
